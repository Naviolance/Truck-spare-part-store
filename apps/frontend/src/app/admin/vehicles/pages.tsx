...
export default function AdminVehiclesPage() {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleAdd} className="grid grid-cols-2 gap-3 mb-6 bg-white border border-gray-200 rounded-lg p-4">
        <div>
          <label className="block text-xs font-medium mb-1">Manufacturer</label>
          <input required value={form.manufacturer} onChange={(e) => update("manufacturer", e.target.value)} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1">Model</label>
          <input required value={form.model} onChange={(e) => update("model", e.target.value)} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1">Year start</label>
          <input required type="number" value={form.yearStart} onChange={(e) => update("yearStart", e.target.value)} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1">Year end (optional)</label>
          <input type="number" value={form.yearEnd} onChange={(e) => update("yearEnd", e.target.value)} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" />
        </div>
        <div className="col-span-2">
          <label className="block text-xs font-medium mb-1">Engine (optional)</label>
          <input value={form.engine} onChange={(e) => update("engine", e.target.value)} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" />
        </div>
        {error && <p className="col-span-2 text-red-600 text-sm">{error}</p>}
        <button className="col-span-2 bg-gray-900 text-white rounded py-2 text-sm">Add vehicle</button>
      </form>
        ...
        {vehicles.map((v) => (
          <li key={v.id} className="flex items-center justify-between p-3 text-sm">
            <span>
              ...
              {v.engine && ` · ${v.engine}`}
              <span className="text-gray-400"> — {v._count.compatibilities} products</span>
            </span>
