...
export default function AdminRequestsPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-zinc-900 tracking-tight mb-6">Product requests</h1>
            ...
            <div key={r.id} className="bg-white border border-zinc-200 rounded-lg p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <p className="text-sm text-zinc-900">{r.description}</p>
                  {r.partNumber && <p className="text-xs text-zinc-500 mt-1">Part number: {r.partNumber}</p>}
                  {r.vehicleInfo && <p className="text-xs text-zinc-500">Vehicle: {r.vehicleInfo}</p>}
                  <p className="text-xs text-zinc-400 mt-1">
                    {r.user.firstName} {r.user.lastName} ({r.user.email}) · {new Date(r.createdAt).toLocaleDateString()}
                  </p>
                </div>
                <div className="shrink-0 flex flex-col items-end gap-2">
                  <span className={`text-xs px-2 py-1 rounded-full ${STATUS_COLORS[r.status] || "bg-zinc-100"}`}>
                    {r.status.replace("_", " ")}
                  </span>
                  <select
                    value={r.status}
              ...
              <textarea
                ...
                rows={2}
                className="w-full mt-3 border border-zinc-200 rounded-lg px-3 py-2 text-sm text-zinc-700 transition-colors duration-200 focus:outline-none focus:border-zinc-400"
              />
            </div>
          ))}
        </div>
