  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/admin/products/new" className="bg-gray-900 text-white text-sm px-4 py-2 rounded">+ New product</Link>
      </div>
      {outOfStock.length > 0 && (
      ...
      <table className="w-full text-sm bg-white border border-gray-200 rounded-lg overflow-hidden">
        ...
        <tbody>
          {products.map((p) => (
            <tr key={p.id} className={`border-t border-gray-100 ${p.quantity === 0 ? "bg-red-50" : ""}`}>
              ...
              <td className="p-3">
                <button onClick={() => togglePublish(p)}
                  className={`text-xs px-2 py-1 rounded-full ${p.status === "PUBLISHED" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}>
                  {p.status}
                </button>
              </td>
